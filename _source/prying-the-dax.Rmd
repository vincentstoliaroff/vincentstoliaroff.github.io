...
The most ubiquitous (in news report) is the *performance-index* quoting as of mid-2016 between 10'000 and 11'000 Points. 
The Definition of the index is based on the _Laspeyres_ Price Index methodologie. The relative weights of the Index component are based on their  
...
The DAX components are daily published as XL files on the website of the [Deutsche Börse](<http://www.dax-indices.com/EN/index.aspx?pageID=4).
...
The Isin Code, Trading Ticker and Name of a given company are not necessary stable over time. For example, Adidas was labelled as _Adidas AG O.N._ till the 8th of October 2010 and _Adidas AG NA O.N._ afterwards. Since we would like to use those 3 variables as primary keys it is worth checking every change in it for chaining consistency. 

##### Number of Constituent and In/Out Reshuffling

Over time the Index Sponsor can decide to _reshuffle_ the Index. 
Checking the time series of each constituent, we find out that a few in/out updates occured in the last years. 

On 2012/09/21 -> 2012/09/24
Out:
MAN SE ST O.N.
METRO AG ST O.N.
In: 
...
# simulate a weighing factor time serie for a given Stock
stock.weight.lagged<-data.frame(Weight=c(3,3,2,2,2,5,5,3,3,6),
                       Datum=seq(as.Date("2010-10-01"),as.Date("2010-10-10"),by = "day"))
                                  
# what we would like
stock.weight.correct<-data.frame(Weight=c(2,2,5,5,5,3,3,6,6,NA),
                                 Datum=seq(as.Date("2010-10-01"),as.Date("2010-10-10"),by = "day"))

ggplot2::qplot(data=stock.weight.lagged,x=Datum,y=Weight,geom="line")
...
The painful task of extracting and cleansing the data is now over... Let's start having fun now with mining, statistics and trading...
